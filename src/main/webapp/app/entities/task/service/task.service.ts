...
export class TaskService {
  ...
  protected convertDateFromClient(task: ITask): ITask {
    return Object.assign({}, task, {
      createdDate: task.createdDate?.isValid() ? task.createdDate.toJSON() : undefined,
      modifiedDate: task.modifiedDate?.isValid() ? task.modifiedDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((task: ITask) => {
        task.createdDate = task.createdDate ? dayjs(task.createdDate) : undefined;
        task.modifiedDate = task.modifiedDate ? dayjs(task.modifiedDate) : undefined;
      });
    }
    return res;
  }
